...
class DataLoader_Global:
    ...
    def _float_feature_nparray(self, value):
    ...
    def generate_data(self, datatype='train',
        ...
        if load_saved_data and not self.test_model:
            ...
            with tf.io.TFRecordWriter('./data/{}_{}.tfrecords'.format(self.dataset, datatype)) as writer:
                for t in range(time_start, time_end):
                    ...
                    for r in range(data_shape[1]):
                        for c in range(data_shape[2]):
                            ...
                            dec_inp_f_sample = f_data[t - 1: t + n_pred - 1, r, c, :]
                            ...
                            dec_inp_ex_sample = ex_data[t - 1: t + n_pred - 1, :]

                            """ generating the ground truth for each sample """
                            y = f_data[t: t + n_pred, r, c, :]
                            ...
                            feature = {
                                'inp_ft': self._float_feature_nparray(np.array(inp_ft_sample, dtype=np.float32)),
                                'inp_ex': self._float_feature_nparray(np.array(inp_ex_sample, dtype=np.float32)),
                                'dec_inp_f': self._float_feature_nparray(np.array(dec_inp_f_sample, dtype=np.float32)),
                                'dec_inp_t': self._float_feature_nparray(np.array(dec_inp_t_sample, dtype=np.float32)),
                                'dec_inp_ex': self._float_feature_nparray(np.array(dec_inp_ex_sample, dtype=np.float32)),
                                'cors': self._float_feature_nparray(np.array(self.cor_gen.get(r, c), dtype=np.float32)),
                                'y': self._float_feature_nparray(np.array(y, dtype=np.float32)),
                                'y_t': self._float_feature_nparray(np.array(tar_t, dtype=np.float32))
                            }
